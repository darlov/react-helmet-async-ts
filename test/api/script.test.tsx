  ...
  describe("Declarative API", () => {
    it("updates script tags", () => {
      ...
      const filteredTags = [...existingTags].filter(
        tag =>
          (tag.src === "http://localhost/test.js" && tag.type === "text/javascript")
          || (tag.src === "http://localhost/test2.js" && tag.type === "text/javascript")
          || (tag.type === "application/ld+json" && tag.innerHTML === scriptInnerHTML)
      );

      expect(filteredTags.length).to.equals(3);
    });
    ...
    it("sets script tags based on deepest nested component", () => {
      ...
      const tagNodes = document.head.querySelectorAll<HTMLScriptElement>(`script[${HELMET_ATTRIBUTE}]`);
      ...
      expect(firstTag.src).toBe("http://localhost/test.js");
      expect(firstTag.type).toBe("text/javascript");
      expect(firstTag.outerHTML).toMatchSnapshot();

      expect(secondTag).toBeInstanceOf(Element);
      expect(secondTag.src).toBe("http://localhost/test2.js");
      expect(secondTag.type).toBe("text/javascript");
